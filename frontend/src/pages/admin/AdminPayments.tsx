...
export default function AdminPayments() {
  ...
  return (
    <div className="p-8">
      <h1 className="font-heading font-extrabold text-2xl text-text mb-1">Verifikasi Pembayaran</h1>
      ...
      <div className="grid gap-4">
        {data?.payments.map((p) => (
          <div key={p.id} className="bg-card border border-border rounded-2xl p-5 flex gap-5">
            <div className="w-28 h-28 rounded-lg bg-section shrink-0 overflow-hidden flex items-center justify-center">
              {p.proof_url ? (
                <a href={`${apiOrigin}${p.proof_url}`} target="_blank" rel="noreferrer">
            ...
            <div className="flex flex-col gap-2 shrink-0">
              <button
                ...
                className="flex items-center gap-2 font-heading font-medium text-sm bg-primary hover:bg-primary-hover text-white rounded-lg px-4 py-2 transition-colors disabled:opacity-60"
              >
              ...
              <button
                ...
                disabled={verifyPayment.isPending}
                className="flex items-center gap-2 font-heading font-medium text-sm border border-error text-error hover:bg-error/10 rounded-lg px-4 py-2 transition-colors disabled:opacity-60"
              >
